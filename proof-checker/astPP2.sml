...
fun dtypePrint modn sys d t pg str brk blk =
  let
    ...
    |   printTerms (x::xs) pos = printTerms [x] pos >> printTerms xs (pos+1)
  in
    printTerms dtypelist 0 >>str ";"
  end;
...
(*Fix these names*)
val _=add_astPP("wordtypeprint",``TC_word64``,genPrint (deftypePrint "word"));
...
val _=add_astPP ("listappendprint", ``App ListAppend [x;y]``,genPrint (infixrealPrint "@"));
